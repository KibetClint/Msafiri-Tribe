      ...
      <nav className="sticky top-0 z-50 bg-safari-cream backdrop-blur-md border-b border-safari-green/20">
        <div className="container flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center gap-1">
            <Link
              to="/"
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${isActive("/") ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-muted"}`}>
              Home
            </Link>
        ...
        {open && (
          <div className="lg:hidden border-t bg-background max-h-[80vh] overflow-y-auto">
            <Link
              ...
              className={`block px-6 py-3 text-sm font-medium ${isActive("/") ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-muted"}`}>
              Home
            </Link>
            ...
            <button
              ...
              <ChevronDown
                size={14}
                className={`transition-transform ${mobileExpanded === "dest" ? "rotate-180" : ""}`}
              />
            </button>
            {mobileExpanded === "dest" && (
              <div className="bg-muted/50">
            ...
            <button
              ...
              <ChevronDown
                size={14}
                className={`transition-transform ${mobileExpanded === "trip" ? "rotate-180" : ""}`}
              />
            </button>
            {mobileExpanded === "trip" && (
              <div className="bg-muted/50">
            ...
            {simpleLinks
              ...
              .map((link) => (
                <Link
                  ...
                  to={link.path}
                  onClick={() => setOpen(false)}
                  className={`block px-6 py-3 text-sm font-medium ${isActive(link.path) ? "bg-primary text-primary-foreground" : "text-foreground hover:bg-muted"}`}>
                  {link.label}
                </Link>
              ))}
          </div>
