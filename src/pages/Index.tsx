...
const iconMap: Record<string, React.ElementType> = {
  Heart, Umbrella, Binoculars, Mountain, Landmark, Crown,
};
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {destinations.slice(0, 8).map((d) => (
              <AnimatedSection key={d.id} delay={(destinations.slice(0, 8).indexOf(d) % 4) * 100}>
      ...
      <section className="py-20">
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {destinations.filter(d => d.category === "international").slice(0, 6).map((d) => (
              <AnimatedSection key={d.id} delay={(destinations.filter(dd => dd.category === "international").slice(0, 6).indexOf(d) % 3) * 120}>
                <Link to={`/destinations/${d.slug}`} className="group block">
                  <Card className="overflow-hidden border-none shadow-md hover:shadow-xl transition-shadow">
                    <div className="relative overflow-hidden aspect-video">
                      <img src={d.image} alt={d.name} loading="lazy" width={1024} height={768} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    </div>
                    <CardContent className="p-6">
                      ...
                      <p className="text-2xl font-bold text-safari-warm">From ${d.packages.basic.price}</p>
                    </CardContent>
                  </Card>
                </Link>
              </AnimatedSection>
            ...
            {testimonials.slice(0, 3).map((t, i) => (
              <AnimatedSection key={i}>
                <div className="bg-primary-foreground/5 rounded-lg p-8 border border-primary-foreground/10">
                  ...
                  <p className="font-display font-semibold text-safari-gold">— {t.author}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
